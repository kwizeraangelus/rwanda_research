...
  
  // CORRECTED: Removed TypeScript type annotations (<any> and <string | null>)
  // The state variables will now be inferred as null, true, and null (string).
...
      if (!id) return;
      
      // Ensure ID is a string for the fetch call
      const bookId = Array.isArray(id) ? id[0] : id;

      try {
        const res = await fetch(`http://127.0.0.1:8000/api/innovations/public-detail/${bookId}/`);
        
        if (res.status === 404) {
...
  const openPDF = () => {
    // Check if the book object exists and has a file_url property
...
  
  // Safety check: if loading is done but no book data was set
